...
sub getCfg {
    ...
    elsif($self->{name} eq "ntperf64") {
        return 1;
    }
    elsif($self->{name} eq "fault_correlation_engine") {
        $directory = "probes/service/fault_correlation"
    }
    elsif($self->{name} eq "alarm_enrichment") {
        $directory = "probes/service/nas/alarm_enrichment"
    }
    ...
    pdsPut_PCH ($PDS,"file","$name.cfg");
    pdsPut_INT ($PDS,"buffer_size",10000000);

    my ($RC, $RES) = nimNamedRequest("$self->{addr}/controller", "text_file_get", $PDS,5);
    pdsDelete($PDS);
